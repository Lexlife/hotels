...
function convertArrayToString(arrayOfCities) {
...
function getNumberRoundedUpToFives(number) {
...
function identifyPrimeNumber(number) {
    if (number < 2) {
...
function getMatchesInArrays(arrayOne, arrayTwo) {
    // определяем массив с выходными данными текущей функции
    let result;
    // определяем массивы для записи уникальных элементов
    const UniqueElementsOfArrayOne = [];
    const UniqueElementsOfArrayTwo = [];
