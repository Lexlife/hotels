...
```text
Расставить можно с помощью основной теоремы арифметики и соотношения a/c = d/b = e/f
(каждое натуральное число можно представить в виде единственного произведения простых множителей).

У нас есть числа, которые содержат простые множители 2^7, 3^4, 5^2, 7, 11

7, 9, 11 - располагаем в диагональных элементах, поскольку с ними соотношения не будут соблюдаться.

Анализ оставшихся чисел показывает, что соотношение равно строго 2, откуда следует:
...
```text
Если бы предметов было бы неограниченное количество, то вполне можно было бы воспользоваться алгоритмом бинарного поиска
(скинуть с середины и отбросить половину, etc).
В данной же ситуации, самым долгим (но экономичным способом) будет алгоритм линейного поиска (сперва сбросить предмет
с одного метра, потом с двух, потом с трёх, etc).
Ещё можно предположить вариант, который сочетает в себе бинарный и линейные алгоритмы. Например, 
